...
CREATE POLICY "cc_select" ON public.credit_cards FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "cc_insert" ON public.credit_cards FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "cc_update" ON public.credit_cards FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "cc_delete" ON public.credit_cards FOR DELETE USING (auth.uid() = user_id);
...
DROP POLICY IF EXISTS "fe_select" ON public.fixed_expenses;
DROP POLICY IF EXISTS "fe_insert" ON public.fixed_expenses;
DROP POLICY IF EXISTS "fe_update" ON public.fixed_expenses;
DROP POLICY IF EXISTS "fe_delete" ON public.fixed_expenses;

CREATE POLICY "fe_select" ON public.fixed_expenses FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "fe_insert" ON public.fixed_expenses FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "fe_update" ON public.fixed_expenses FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "fe_delete" ON public.fixed_expenses FOR DELETE USING (auth.uid() = user_id);
